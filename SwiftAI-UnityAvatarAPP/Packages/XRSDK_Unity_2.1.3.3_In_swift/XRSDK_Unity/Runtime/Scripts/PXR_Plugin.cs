...
namespace Unity.XR.PXR
{
    ...
    // imu data
    public struct BodyTrackerImu
    {
        public Int64 TimeStamp;                // time stamp of imu
        public double temperature;              // temperature of imu
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
        public double[] GyroData;              // gyroscope data, x,y,z
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
        public double[] AccData;               // Accelerometer data, x,y,z
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
        public double[] MagData;               // magnetometer data, x,y,z
    }
    ...
    {
        private const string PXR_SDK_Version = "2.1.3.3";
		...
		//Body tracking
        [DllImport(PXR_API_DLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int Pxr_SetBodyTrackingStaticCalibState(int state);

        [DllImport(PXR_API_DLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int Pxr_GetBodyTrackingPose(double predictTime, ref BodyTrackerResult bodyTrackerResult);

        [DllImport(PXR_API_DLL, CallingConvention = CallingConvention.Cdecl)]
        public static extern int Pxr_GetBodyTrackingImuData(UInt32 deviceID, ref BodyTrackerImu bodyTrackerImu);
                    ...
                    using (AndroidJavaObject joActivity = jcPlayer.GetStatic<AndroidJavaObject>("currentActivity"))
                    {
                        using (AndroidJavaObject joPackageManager = joActivity.Call<AndroidJavaObject>("getPackageManager"))
                        {
                            using (AndroidJavaObject joIntent = joPackageManager.Call<AndroidJavaObject>("getLaunchIntentForPackage", pkgName))
                            {
                                if (null != joIntent)
                                {
                                    Debug.Log("PXR_UPxr_OpenFitnessBandCalibrationPackage :" + Application.identifier);
        ...
        public static class Controller
        {
            ...
            public static int UPxr_SetBodyTrackingStaticCalibState(int state)
            {
#if UNITY_ANDROID && !UNITY_EDITOR
                return Pxr_SetBodyTrackingStaticCalibState(state);
#endif
                return 1;
            }
            ...
            public static int UPxr_GetBodyTrackingImuData(UInt32 deviceID, ref BodyTrackerImu bodyTrackerImu)
            {
#if UNITY_ANDROID && !UNITY_EDITOR
                return Pxr_GetBodyTrackingImuData(deviceID, ref bodyTrackerImu);
                bodyTrackerImu.GyroData[2] = -bodyTrackerImu.GyroData[2];
                bodyTrackerImu.AccData[2] = -bodyTrackerImu.AccData[2];
                bodyTrackerImu.MagData[2] = -bodyTrackerImu.MagData[2];
#endif
                return 1;
            }
